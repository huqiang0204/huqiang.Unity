    ...
    public class UIDate : ModelInital
    {
        ...
        public string YearUnit
        {
            ...
            set
            {
                ...
                {
                    var it = its[i];
                    var ele = Year.Items[i].obj as Text;
                    if (ele != null)
                    {
        ...
        void UpdateItems(ScrollY scroll)
        {
            ...
            {
                var mod = items[i].target.transform as RectTransform;
                float h = mod.sizeDelta.y;
                float y = mod.localPosition.y;
                float angle = y / h * 15f;
                mod.localRotation = Quaternion.Euler(angle, 0, 0);

                var v = MathH.Tan2(90 - angle);
                var pos = mod.localPosition;
                pos.y = v.y * 100;
                mod.localPosition = pos;
