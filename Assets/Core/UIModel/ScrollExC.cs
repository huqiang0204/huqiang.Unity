...
namespace huqiang.UIModel
{
    public class ScrollExC : ModelInital
    {
        ...
        public ScrollExC()
        { }
        public ScrollExC(RectTransform con)
        {
            Initial(con, null);
        }
        ...
        void Initialtems()
        {
            ...
            for (int i = Items.Count - 1; i >= c; i--)
            {
                var g = Items[i];
                Items.RemoveAt(i);
                ModelManager.RecycleGameObject(g.target);
            }
        }
        ...
        Item GetTitle(int index)
        {
            while (index >= Titles.Count)
            {
                ...
                if (TitleMod != null)
                {
                    i.target = ModelManager.LoadToGame(TitleMod, null, null, "");
        ...
        Item GetTail(int index)
        {
            while (index >= Tails.Count)
            {
                ...
                if (TailMod != null)
                {
                    i.target = ModelManager.LoadToGame(TailMod, null, null, "");
                    var t = i.target.transform;
                    t.SetParent(View);
                    t.localScale = new Vector3(1, 1, 1);
                }
        ...
        {
            for (int i = 0; i < Titles.Count; i++)
                ModelManager.RecycleGameObject(Titles[i].target);
            for (int i = 0; i < Items.Count; i++)
