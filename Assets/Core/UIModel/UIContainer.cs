...
namespace huqiang
{
    ...
    public class UILinker<T, U> : Linker where T : class, new() where U : class, new()
    {
        ...
        public LinkerMod CreateUI()
        {
            ...
            mod.main = ModelManager.LoadToGame(model, ui, null, "");
            mod.UI = ui;
            return mod;
        }
    ...
    public class ObjectLinker : Linker 
    {
        ...
        {
            LinkerMod mod = new LinkerMod();
            mod.main = ModelManager.LoadToGame(model, null, null, "");
            if (ItemCreate != null)
                ItemCreate(this,mod);
            return mod;
        }
    ...
    public class UIContainer
    {
        ...
        void Order()
        {
            ...
            for (int i = c; i >= 0; i--)
            {
                ...
                if (sy - point > vy)
                {
                    ModelManager.RecycleGameObject(item.main);
                    items.RemoveAt(i);
                }
                else if (ey < point)
                {
        ...
        {
            for (int i = 0; i < items.Count; i++)
                ModelManager.RecycleGameObject(items[i].main);
            items.Clear();
            point = 0;
            start = 0;
            end = 0;
